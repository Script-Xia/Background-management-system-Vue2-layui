      ...
      <div class="search-container">
        <div class="layui-form">
          ...
          <input
            ...
            placeholder="请输入搜索内容"
            autocomplete="on"
            class="layui-input serach-value"
          />
        </div>
        <button
    ...
    return {
      ...
      tableCols: [
        [
          ...
          { field: 'city', width: 115, title: '城市', minWidth: 115 },
          {
            field: 'sign',
            title: '签名',
            minWidth: 260,
  ...
  methods: {
    ...
    deleteTableRow(data, table) {
      ...
      layer.confirm('确认删除所选内容吗?', { icon: 3, title: '确认' }, function(
        ...
        const afterDeleteTable = this.tableData.data.filter(item => {
          return idList.indexOf(item.id) == -1
        })

        table.reload(this.tableData.id, { data: afterDeleteTable })
    ...
    addOptionsTemplate(laytpl, selector, data, optionValueKey, optionTitleKey) {
  ...
  mounted() {
    ...
    layui.use(['table', 'form', 'jquery', 'laytpl', 'laydate'], function() {
      ...
      form.on('select(province-select)', function(data) {
        const provinceData = cityData.filter(city => city.name === data.value)
        that.bindCityOptions(laytpl, provinceData[0].citys)
      ...
      table.on('tool(tableDemo)', function(obj) {
        ...
        if (obj.event === 'del') {
          ...
          data['city[province]'] = data.city[0]
          data['city[city]'] = data.city[1]
          delete data.city
          delete data.hobby
          console.log(data);
